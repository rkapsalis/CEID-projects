...
/*
	...
	vector<Point>   polyline1,simplified_polyline1;
	#pragma omp parallel
	{
        int numb = static_cast<int>(AllPolylines.size())/omp_get_num_threads();
	 double wtime = omp_get_wtime();
       #pragma omp for ordered schedule(static,numb) nowait

	
	
	for (int i = 0; i < static_cast<int>(AllPolylines.size()); i++) {
		
		

		polyline1 = AllPolylines.at(i);
		
		
		RamerDouglasPeucker(polyline1, epsilon, simplified_polyline1);
	...
	if (print_results != 0) {
		...
		for (vector< vector<Point> >::const_iterator it1 = AllPolylines.begin(), it2 = SimplifiedAllPolylines.begin();
			...
			for (vector<Point>::const_iterator it3 = simplified_polyline.begin(); it3 != simplified_polyline.end(); it3++) {
				cout << setprecision(numeric_limits<double>::digits10 + 1) << "(" << (*it3).first << ", " << (*it3).second << ") ";
outfile <<setprecision(numeric_limits<double>::digits10 + 1) << "(" << (*it3).first << ", " << (*it3).second << ") " << std::endl;			
}
			cout << endl << endl;
		}
	}
